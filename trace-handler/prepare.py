...
from net_log_event_type_list_119_0_6046_159 import event_type
...
def clean_pcap_csv(csv_path, json_path, n_streams, client_ip="127.0.0.1", server_ip="127.0.0.2", save=False,
    ...
    clean_data = valid_data[['frame.time_relative', 'frame.len', 'ip.src', 'ip.dst', 'Server_src', 'object_started']]

    clean_data.rename(columns={'frame.time_relative': 'Time', 'frame.len': 'Length', 'ip.src': 'Source',
                               'ip.dst': 'Destination'}, inplace=True)
    if save:
