...
def main():
    ...
    while len(files) > 0:
        ...
        requests = [f"""tshark -r {file} -R quic -2 -T fields -e frame.number -e frame.time_relative -e frame.len -e eth.src -e eth.dst -e ip.src -e ip.dst -e ip.proto -e _ws.col.Info -E header=y -E separator=, -E quote=d -E occurrence=f -o tls.keylog_file:{file.replace('.pcap', '.key')} > {file[:-5]}.csv""" for file in files[:num_of_files]]
        print(len(files))
